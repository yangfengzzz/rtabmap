"""
Adapted from https://github.com/byangw/PCT_planner/tree/main/tomography/scripts
...
class Tomogram:
    ...
    def initBuffers(self):
        ...
        self.layers_g = cp.zeros(shape, dtype=cp.float32)
        self.layers_c = cp.zeros(shape, dtype=cp.float32)
        self.grad_mag_sq = cp.zeros(shape, dtype=cp.float32)
        self.grad_mag_max = cp.zeros(shape, dtype=cp.float32)
        self.trav_cost = cp.zeros(shape, dtype=cp.float32)
        self.inflated_cost = cp.zeros(shape, dtype=cp.float32)
    ...
    def point2map(self, points):
        ...
        self.tomography_kernel(points, self.center, self.layers_g, self.layers_c, size=(points.shape[0]))
